...
export class ClienteService {
  ...
  getCliente(id: number): Observable<any> {
    ...
    return this.http.get<Cliente>(url).pipe(map( any => {
      ...
      this.domicilioService.getDomicilio(any.idDomicilio).subscribe(dom =>{
  ...
  getClientes(id: number,apellido: string, nombre: string, tipoDeDocumento: string, numeroDeDocumento: number): Observable<any>{
    ...
    if(tipoDeDocumento != undefined) url +=`tipoDeDocumento=${tipoDeDocumento}`;
    if(numeroDeDocumento != undefined) url +=`numeroDeDocumento=${(numeroDeDocumento.toString())}`;
    return this.http.get<any>(url).pipe(map(any => {
     // console.log("Cliente recibidio: "+response);
      ...
      clientes.forEach( cliente => {
        this.domicilioService.getDomicilio(cliente.idDomicilio).subscribe(dom =>{
          console.log("Domicilio obtenido buscando cliente: "+ dom);
          cliente.domicilio = new Domicilio(dom)},
        error => {console.log("No se puede obtener el domicilio");
        }); //TODO deberia ser un DTOCliente lo que se recibe!!!!
